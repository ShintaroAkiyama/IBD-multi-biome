...
Meta %>% filter(Meta$UC_Dx == 1 & Meta$TNF == 0 | Meta$UC_Dx ==0) -> Meta #UC without TNF and control
...
SP_input2 <- SP_input[, -which (colnames(SP_input) %in% c("ID", "IBD_HC2", "Age", "sex", "BMI", "IBD_HC2.1", "UC.x", 
                                                          ...
                                                          "Infliximab.or.adalimumab", "Any.clinical.symptoms", "TNF", "IBD.y", "UC.y", "CD"))] 
...
JP_maaslin %>% filter(JP_maaslin$"metadata" == "UC_Dx") -> JP_maaslin_UC
JP_maaslin_UC <- JP_maaslin_UC[, -which (colnames(JP_maaslin_UC) %in% c("metadata", "value", "stderr", "name", "N", "N.not.zero"))] 
rename(.data=JP_maaslin_UC, "Coefficient (UC w/o TNF/IM)" = "coef") -> JP_maaslin_UC
rename(.data=JP_maaslin_UC, "P-value (UC w/o TNF/IM)" = "pval") -> JP_maaslin_UC
rename(.data=JP_maaslin_UC, "Q-value (UC w/o TNF/IM)" = "qval") -> JP_maaslin_UC

#CDvsControl
Meta <- read.csv("Meta.csv", header = TRUE, na.strings = c(NA, ''), row.names=1)
Meta %>% mutate("TNF" = if_else(Meta$"Infliximab.or.adalimumab" == 1 |Meta$"Immunomodulators" == 1, 1, 0)) -> Meta

Meta %>% filter(Meta$CD_Dx == 1 & Meta$TNF == 0 | Meta$CD_Dx ==0) -> Meta #CD without TNF and control
rownames_to_column(Meta, "METAF") -> Meta
...
JP_maaslin %>% filter(JP_maaslin$"metadata" == "CD_Dx") -> JP_maaslin_CD
JP_maaslin_CD <- JP_maaslin_CD[, -which (colnames(JP_maaslin_CD) %in% c("metadata", "value", "stderr", "name", "N", "N.not.zero"))] 
rename(.data=JP_maaslin_CD, "Coefficient (CD w/o TNF/IM)" = "coef") -> JP_maaslin_CD
rename(.data=JP_maaslin_CD, "P-value (CD w/o TNF/IM)" = "pval") -> JP_maaslin_CD
rename(.data=JP_maaslin_CD, "Q-value (CD w/o TNF/IM)" = "qval") -> JP_maaslin_CD

#IBDvsControl
Meta <- read.csv("Meta.csv", header = TRUE, na.strings = c(NA, ''), row.names=1)
Meta %>% mutate("TNF" = if_else(Meta$"Infliximab.or.adalimumab" == 1 |Meta$"Immunomodulators" == 1, 1, 0)) -> Meta

Meta %>% filter(Meta$IBD == 1 & Meta$TNF == 0 | Meta$IBD ==0) -> Meta #IBD without TNF and control
rownames_to_column(Meta, "METAF") -> Meta
...
JP_maaslin %>% filter(JP_maaslin$"metadata" == "IBD") -> JP_maaslin_IBD
JP_maaslin_IBD <- JP_maaslin_IBD[, -which (colnames(JP_maaslin_IBD) %in% c("metadata", "value", "stderr", "name", "N", "N.not.zero"))] 
rename(.data=JP_maaslin_IBD, "Coefficient (IBD w/o TNF/IM)" = "coef") -> JP_maaslin_IBD
rename(.data=JP_maaslin_IBD, "P-value (IBD w/o TNF/IM)" = "pval") -> JP_maaslin_IBD
rename(.data=JP_maaslin_IBD, "Q-value (IBD w/o TNF/IM)" = "qval") -> JP_maaslin_IBD

#Data integration
JP_MA_TNF <- full_join(full_join(JP_maaslin_IBD, JP_maaslin_UC, by = "feature"), JP_maaslin_CD, by = "feature")
JP_MA_TNF %>% mutate(ID = str_sub(JP_MA_TNF$feature, start = -6, end = -2)) -> JP_MA_TNF
...
sp <- ggplot(JP_MA2, aes(x = JP_MA2$"Coefficient (IBD JP)", y = JP_MA2$"Coefficient (IBD w/o TNF/IM)")) +
  ...
  labs(title = "", subtitle = "IBD", x = "Coefficient value (all IBD)", y = "Coefficient value (IBD w/o TNF/AZA)", tag = "") +
...
sp <- ggplot(JP_MA2, aes(x = JP_MA2$"Coefficient (UC JP)", y = JP_MA2$"Coefficient (UC w/o TNF/IM)")) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "lm") +
  labs(title = "", subtitle = "UC", x = "Coefficient value (all UC)", y = "Coefficient value (UC w/o TNF/AZA)", tag = "") +
  theme(plot.title = element_text(face = "bold", color = "black"), plot.tag  = element_text(face = "bold", color = "black", size =24))
sp_uc <- sp +  stat_cor(method = "spearman", label.x = -2, label.y = 2, cor.coef.name = c("rho"))

sp <- ggplot(JP_MA2, aes(x = JP_MA2$"Coefficient (CD JP)", y = JP_MA2$"Coefficient (CD w/o TNF/IM)")) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "lm") +
  labs(title = "", subtitle = "CD", x = "Coefficient value (all CD)", y = "Coefficient value (CD w/o TNF/AZA)", tag = "") +
  theme(plot.title = element_text(face = "bold", color = "black"), plot.tag  = element_text(face = "bold", color = "black", size =24))
...
combine_plots(
  ...
  annotation.args = list(
    title = "Sensitivity analysis (all IBD vs IBD without TNF inhibitors/azathioprine)",
    caption = ""
  )
)
...
SP_input2_bacteriome %>% select("Coefficient (IBD JP)", "P-value (IBD JP)", "Q-value (IBD JP)", "Coefficient (UC JP)", "P-value (UC JP)", "Q-value (UC JP)", "Coefficient (CD JP)", "P-value (CD JP)", "Q-value (CD JP)", 
                                "Coefficient (IBD w/o TNF/IM)", "P-value (IBD w/o TNF/IM)", "Q-value (IBD w/o TNF/IM)", "Coefficient (UC w/o TNF/IM)", "P-value (UC w/o TNF/IM)", "Q-value (UC w/o TNF/IM)", 
                                "Coefficient (CD w/o TNF/IM)", "P-value (CD w/o TNF/IM)", "Q-value (CD w/o TNF/IM)") ->  Fig1 

Fig1 %>% select("Q-value (IBD JP)", "Q-value (IBD w/o TNF/IM)") -> qval_IBD
Fig1 %>% select("Q-value (UC JP)",  "Q-value (UC w/o TNF/IM)") -> qval_UC
Fig1 %>% select("Q-value (CD JP)", "Q-value (CD w/o TNF/IM)") -> qval_CD

qval_IBD[is.na(qval_IBD)] <- 1
qval_UC[is.na(qval_UC)] <- 1
qval_CD[is.na(qval_CD)] <- 1

Fig1 %>% select("Coefficient (IBD JP)", "Coefficient (IBD w/o TNF/IM)") -> Gram_coef_IBD
Fig1 %>% select("Coefficient (UC JP)",  "Coefficient (UC w/o TNF/IM)") -> Gram_coef_UC
Fig1 %>% select("Coefficient (CD JP)", "Coefficient (CD w/o TNF/IM)") -> Gram_coef_CD
