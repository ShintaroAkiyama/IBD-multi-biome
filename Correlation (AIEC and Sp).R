...
#Select KO with coef>0 and FDR<0.1 in UC or CD
...
#Prepare for AIEC virulence factor abundance data
KO_coef %>% filter(!is.na(KO_coef$AIEC)) -> KO_coef
KO_coef %>% arrange(-KO_coef$"Coefficient (IBD JP)") -> KO_coef
KO_coef %>% arrange(KO_coef$AIEC) -> KO_coef
...
left_join(KO_coef, KO_input2, by = "feature") -> KO_input2
...
KO_input2  <- KO_input2[, !is.na(colSums(KO_input2))] 
...
# Reorder the rho data according to the AIEC coef (IBD)
...
T_rho %>% arrange(T_rho$"AIEC") -> T_rho
...
T_rho %>% select(feature) -> T_rho_KO_p #List of selected KO (AIEC)
column_to_rownames(T_rho, var="feature") -> T_rho
T_rho <- as.data.frame(t(T_rho))

# Reorder the adjusted P data according to the AIEC coef (IBD)
...
K_p$"Function"[is.na(K_p$"Function")] <- "Others" #Function NA is substituted with Others
KO_ra <- HeatmapAnnotation("MaAsLin (UC vs HC)" = anno_simple(K_p$"Coefficient (UC JP)", col = m_col_fun, pch = pch2,  simple_anno_size = unit(3, "mm"), gp = gpar(col = "black")), 
                           "MaAsLin (CD vs HC)" = anno_simple(K_p$"Coefficient (CD JP)", col = m_col_fun, pch = pch3,  simple_anno_size = unit(3, "mm"), gp = gpar(col = "black")), 
                           "AIEC virulence factors"=K_p$AIEC, "Function"=K_p$Function, col=list("AIEC virulence factors"=c("Crossing of mucous layer and host defense peptides" = "#fff100", 
                                                                                                                           "Interaction with Peyer's patches and macrophages survival"="#03af7a", 
                                                                                                                           "Metabolic adaptation and global regulation"="#990099", 
                                                                                                                           "Adhesion to and invasion of IECs"="#ff4b00"),  
                                                                                                "Function"=c("Flagellar assembly" ="#c9ace6", "Fimbriae type 1"="#ff8082", "Type VI secretion"="#ffff80", 
                                                                                                             "Others"="#d8f255")),  simple_anno_size = unit(3,"mm"), annotation_name_gp= gpar(fontsize = 8), gp = gpar(col = "black"))
...
p3=pheatmap(as.matrix(T_rho), fontsize = 5, cellwidth = 8, cellheight = 8, cluster_rows = FALSE, cluster_cols = FALSE, fontsize_col = 8, fontsize_row = 8, display_numbers = matrix(ifelse(T_p <0.1,"*", ""), nrow(T_p)), fontsize_number = 7, border_color = "black", col = circlize::colorRamp2(c(-1,0,1), c("#03af7a", "white", "#f6aa00")), name = "Spearman rho", heatmap_legend_param = list(color_bar = "continuous"), column_title = "AIEC virulence factors", column_title_gp = gpar(fontsize = 10, fontface = "bold"), top_annotation = KO_ra)
