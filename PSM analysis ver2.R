#Propensity score matching analysis (Ref: https://necostat.hatenablog.jp/entry/2022/04/06/001353)
library(dplyr)
library(tidyverse)
...
#Calculate AUC of PS 
ps_model <- glm(IBDforPSM ~ `Age50over` + `sex` + `BMI25over` + `Alchol` + 
...
#ROC
roc_ps <- roc(IBDforPSM ~ ps_logit, data=Meta, ci=TRUE)
ggroc(roc_ps,
      size=1,
      legacy.axes = TRUE) +
...
#PS_logit_matching
matched_2 <- matchit(IBDforPSM ~ `Age50over` + `sex` + `BMI25over` +  `Alchol` + 
                       `CurrentSmoking` + `Bread`+ `Vegetables` + `Fruit` + `Seafood` + 
                       `Meat` + `Egg` + `Coffee` + `StrongActive` + `ModerateActive` + 
                       `WeekActive` +`DM` +`Dyslipidemia` + `Hypertension` + 
                       `AcuteCoronarySyndrome` + `ChronicHeartFaulure` + `CVD` + 
...
#plot
love.plot(matched_2, threshold = 0.1, abs = TRUE, stars="std")

data_ps <- match.data(matched_2) %>% as.data.frame() #Go to the CreateTableOne if table is needed
...
#Calculate AUC of PS
ps_model <- glm(UCforPSM ~ `Age50over` + `sex` + `BMI25over` + `Alchol` + 
                  `CurrentSmoking` + `Bread`+ `Vegetables` + `Fruit` + `Seafood` + 
                  `Meat` + `Egg` + `Coffee` + `StrongActive` + `ModerateActive` + 
                  `WeekActive` +`DM` +`Dyslipidemia` + `Hypertension` + 
                  `AcuteCoronarySyndrome` + `ChronicHeartFaulure` + `CVD` + 
...
roc_ps <- roc(UCforPSM ~ ps_logit, data=Meta, ci=TRUE)
ggroc(roc_ps,
      size=1,
      legacy.axes = TRUE) +
  geom_abline(color="dark grey", size=0.5)
...
#PS_logit_matching
matched_2 <- matchit(UCforPSM ~ `Age50over` + `sex` + `BMI25over` +  `Alchol` + 
                       `CurrentSmoking` + `Bread`+ `Vegetables` + `Fruit` + `Seafood` + 
                       `Meat` + `Egg` + `Coffee` + `StrongActive` + `ModerateActive` + 
                       `WeekActive` +`DM` +`Dyslipidemia` + `Hypertension` + 
                       `AcuteCoronarySyndrome` + `ChronicHeartFaulure` + `CVD` + 
...
#Calculate AUC of PS
ps_model <- glm(CDforPSM ~ `Age50over` + `sex` + `BMI25over` + `Alchol` + 
                  `CurrentSmoking` + `Bread`+ `Vegetables` + `Fruit` + `Seafood` + 
                  `Meat` + `Egg` + `Coffee` + `StrongActive` + `ModerateActive` + 
                  `WeekActive` +`DM` +`Dyslipidemia` + `Hypertension` + 
                  `AcuteCoronarySyndrome` + `ChronicHeartFaulure` + `CVD` + 
...
roc_ps <- roc(CDforPSM ~ ps_logit, data=Meta, ci=TRUE)
ggroc(roc_ps,
      size=1,
      legacy.axes = TRUE) +
  geom_abline(color="dark grey", size=0.5)
...
#PS_logit_matching
matched_2 <- matchit(CDforPSM ~ `Age50over` + `sex` + `BMI25over` +  `Alchol` + 
                       `CurrentSmoking` + `Bread`+ `Vegetables` + `Fruit` + `Seafood` + 
                       `Meat` + `Egg` + `Coffee` + `StrongActive` + `ModerateActive` + 
                       `WeekActive` +`DM` +`Dyslipidemia` + `Hypertension` + 
                       `AcuteCoronarySyndrome` + `ChronicHeartFaulure` + `CVD` + 
...
sp_ibd <- sp + stat_cor(method = "spearman", label.x = -2, label.y = 1, cor.coef.name = c("rho"))
...
sp <- ggplot(All_country, aes(x = All_country$"Coefficient (UC JP)", y = All_country$"Coefficient (UC PSM)")) +
  ...
  theme(plot.title = element_text(face = "bold", color = "black"), plot.tag  = element_text(face = "bold", color = "black", size =24))
sp_uc <- sp + stat_cor(method = "spearman", label.x = -2, label.y = 1, cor.coef.name = c("rho"))
...
sp <- ggplot(All_country, aes(x = All_country$"Coefficient (CD JP)", y = All_country$"Coefficient (CD PSM)")) +
  ...
  theme(plot.title = element_text(face = "bold", color = "black"), plot.tag  = element_text(face = "bold", color = "black", size =24))
sp_cd <- sp + stat_cor(method = "spearman", label.x = -2, label.y = 1, cor.coef.name = c("rho"))
