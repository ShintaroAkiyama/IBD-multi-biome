...
rename(.data= Meta, "Symptoms" = "Any.clinical.symptoms") -> Meta
Meta %>% select(Age, Sex, BMI, "Symptoms") -> JP_meta
...
SP_input2 <- SP_input2[, -which (colnames(SP_input2) %in% c("Age", "Sex", "BMI", "Symptoms", "METAF", "IBD", "UC", "CD"))] 
...
fit_data = Maaslin2(
  ...
  reference = c("Symptoms,0")) 

fit_data$results -> JP_maaslin
JP_maaslin %>% filter(JP_maaslin$"metadata" == "Symptoms") -> JP_maaslin_IBD
JP_maaslin_IBD <- JP_maaslin_IBD[, -which (colnames(JP_maaslin_IBD) %in% c("metadata", "value", "stderr", "name", "N", "N.not.zero"))] 
rename(.data=JP_maaslin_IBD, "Coefficient (IBD clinically active)" = "coef") -> JP_maaslin_IBD
rename(.data=JP_maaslin_IBD, "P-value (IBD clinically active)" = "pval") -> JP_maaslin_IBD
rename(.data=JP_maaslin_IBD, "Q-value (IBD clinically active)" = "qval") -> JP_maaslin_IBD

#UC
Meta <- read.csv("Meta.csv", header = TRUE, na.strings = c(NA, ''), row.names=1)
rename(.data= Meta, "Symptoms" = "Any.clinical.symptoms") -> Meta
Meta %>% select(Age, Sex, BMI, UC, Crohn, "Symptoms") -> JP_meta
...
left_join(JP_meta, SP_input, by = "ID") -> SP_input2
SP_input2 <- SP_input2[, -which (colnames(SP_input2) %in% c("Age", "Sex", "BMI", "UC.x", "Crohn", "Symptoms", "METAF", "IBD", "UC.y", "CD"))] 
...
fit_data$results -> JP_maaslin
JP_maaslin %>% filter(JP_maaslin$"metadata" == "Symptoms") -> JP_maaslin_UC
JP_maaslin_UC <- JP_maaslin_UC[, -which (colnames(JP_maaslin_UC) %in% c("metadata", "value", "stderr", "name", "N", "N.not.zero"))] 
rename(.data=JP_maaslin_UC, "Coefficient (UC clinically active)" = "coef") -> JP_maaslin_UC
rename(.data=JP_maaslin_UC, "P-value (UC clinically active)" = "pval") -> JP_maaslin_UC
rename(.data=JP_maaslin_UC, "Q-value (UC clinically active)" = "qval") -> JP_maaslin_UC
...
fit_data$results -> JP_maaslin
JP_maaslin %>% filter(JP_maaslin$"metadata" == "Symptoms") -> JP_maaslin_CD
JP_maaslin_CD <- JP_maaslin_CD[, -which (colnames(JP_maaslin_CD) %in% c("metadata", "value", "stderr", "name", "N", "N.not.zero"))] 
rename(.data=JP_maaslin_CD, "Coefficient (CD clinically active)" = "coef") -> JP_maaslin_CD
rename(.data=JP_maaslin_CD, "P-value (CD clinically active)" = "pval") -> JP_maaslin_CD
rename(.data=JP_maaslin_CD, "Q-value (CD clinically active)" = "qval") -> JP_maaslin_CD
...
#Heatmap creation
#Select species with abd coef > 0.8 in UC or CD
JP_MA %>% filter(abs(JP_MA$"Coefficient (CD clinically active)") > 0.8 | abs(JP_MA$"Coefficient (UC clinically active)") > 0.8) -> SP_input
SP_input %>% arrange(-SP_input$"Coefficient (IBD clinically active)") -> SP_input2 
column_to_rownames(SP_input2, "Feature") -> SP_input2

SP_input2 %>% select("Coefficient (IBD clinically active)", "P-value (IBD clinically active)", "Q-value (IBD clinically active)", 
                     "Coefficient (UC clinically active)", "P-value (UC clinically active)", "Q-value (UC clinically active)", 
                     "Coefficient (CD clinically active)", "P-value (CD clinically active)", "Q-value (CD clinically active)") ->  Fig1 

Fig1 %>% select("Q-value (IBD clinically active)") -> qval_IBD
Fig1 %>% select("Q-value (UC clinically active)", "Q-value (CD clinically active)") -> qval_UC_CD
qval_IBD[is.na(qval_IBD)] <- 1
qval_UC_CD[is.na(qval_UC_CD)] <- 1

Fig1 %>% select("P-value (IBD clinically active)") -> pval_IBD
Fig1 %>% select("P-value (UC clinically active)", "P-value (CD clinically active)") -> pval_UC_CD
pval_IBD[is.na(pval_IBD)] <- 1
pval_UC_CD[is.na(pval_UC_CD)] <- 1

Fig1 %>% select("Coefficient (IBD clinically active)") -> Gram_coef_IBD
Fig1 %>% select("Coefficient (UC clinically active)", "Coefficient (CD clinically active)") -> Gram_coef_UC_CD
