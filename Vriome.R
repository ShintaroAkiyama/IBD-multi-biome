...
SP_input2 %>% arrange(-SP_input2$"Coefficient (IBD JP)") -> SP_input2 
column_to_rownames(SP_input2, "feature2") -> SP_input2 
SP_input2 %>% select(c(49:58)) -> species_table_joined #Extract predicted host data (need to check the location of host data)
...
#Select species with abs coef >1 and FDR<0.15 in UC or CD 
All_country %>% filter(abs(All_country$"Coefficient (CD JP)") > 1 & All_country$"Q-value (CD JP)" < 0.15 | abs(All_country$"Coefficient (UC JP)") > 1 & All_country$"Q-value (UC JP)" < 0.15) -> SP_input
SP_input %>% arrange(-SP_input$"Coefficient (IBD JP)") -> SP_input2 
